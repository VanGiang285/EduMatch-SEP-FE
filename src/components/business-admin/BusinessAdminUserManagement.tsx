...
import { ROLE_ID_MAP, ROLE_LABELS } from '@/constants';

interface BusinessAdminUserManagementProps {
  className?: string;
}

export default function BusinessAdminUserManagement({ className }: BusinessAdminUserManagementProps) {
  ...
  const getUserRole = (user: ManageUserDto) => {
    // Business Admin can only manage learners and tutors
  ...
  const filteredUsers = users.filter(user => {
    ...
    // Business Admin can only see learners and tutors
  ...
  // Business Admin specific statistics
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center">
              <Users className="h-8 w-8 mr-3 text-[#257180]" />
              Quản lý người dùng
            </h1>
            <p className="text-gray-600 mt-1">
              Quản lý tài khoản học viên và gia sư
            </p>
          </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-[#257180] mt-1">{users.length}</p>
              </div>
