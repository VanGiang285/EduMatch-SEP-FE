...
import { SystemAdminLayout as SystemAdminLayoutComponent } from '@/components/system-admin/layout/SystemAdminLayout';
...
export default function SystemAdminLayout({ children }: SystemAdminLayoutProps) {
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      // If user exists but is not SYSTEM_ADMIN, redirect to unauthorized
      if (user && user.role !== USER_ROLES.SYSTEM_ADMIN && !isOnUnauthorizedPage && !isOnHomePage) {
